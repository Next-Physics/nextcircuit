...
from funcs.planning import create_step_success_check_logic
...
def debug_and_repair_code(d,block_num,block,e,step_num,tb_str,attempts_log):
    ...
    part_one = f"""
    You are an expert at investigating errors, debugging code, and resolving issues in code blocks from plans generated by LLM agent to achieve a users goal.
    ...
    DURING THE CURRENT STEP {step_num}, WHICH CONSISTS OF STEP ELABORATION:
    '''{d['plan']['steps'][step_num]['elaboration']}'''

    The failing code block was:
    ...
    part_two = f"""Please fix this error by rewriting the entire revised elaboration of step {step_num} as needed in order for the code to run without errors.
    ...
    - Consider if the previous step in the plan could have failed and if so, correct it before this step.

    Return the fully revised elaboration of step {step_num}:
    ...
    # 
    d["plan"]["steps"][i]["success_check"] = create_step_success_check_logic(step_num,d)
...
def run_code_blocks(d,code_blocks, step_num,attempts_log):
    ...
    # Return True if the code block was successfully executed
           ...
           # traceback_str = ''.join(traceback.format_tb(e.__traceback__))
            ...
            debug_and_repair_code(d,block_num,block,e,step_num,tb_str,attempts_log)
            return False
